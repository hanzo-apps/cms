...
import { mongooseAdapter } from '@hanzo/cms-db-mongodb'`,
    ...
    const result = findImportDeclaration({ sourceFile, moduleSpecifier: '@hanzo/cms-db-mongodb' })

    expect(result).toBeDefined()
    expect(result?.getModuleSpecifierValue()).toBe('@hanzo/cms-db-mongodb')
  })
})
...
describe('addImportDeclaration', () => {
  it('adds new import when not present', () => {
    ...
    const result = addImportDeclaration({
      sourceFile,
      moduleSpecifier: '@hanzo/cms-db-postgres',
      namedImports: ['postgresAdapter'],
    })

    const imports = result.getImportDeclarations()
    expect(imports).toHaveLength(2)
    expect(imports[1].getModuleSpecifierValue()).toBe('@hanzo/cms-db-postgres')
  ...
  it('does not duplicate existing import', () => {
    ...
    const result = addImportDeclaration({
      sourceFile,
      moduleSpecifier: '@hanzo/cms-db-mongodb',
...
describe('removeNamedImports', () => {
  it('removes specific named imports', () => {
    ...
    const sourceFile = project.createSourceFile(
      'test.ts',
      `import { mongooseAdapter, SomeOtherType } from '@hanzo/cms-db-mongodb'`,
    )
...
export default buildConfig({
  db: postgresAdapter({ url: '' })
    ...
    const imports = result.sourceFile.getImportDeclarations()
    expect(imports).toHaveLength(1)
    expect(imports[0].getModuleSpecifierValue()).toBe('@hanzo/cms-db-postgres')
  })
