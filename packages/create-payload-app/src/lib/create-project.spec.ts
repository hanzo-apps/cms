  ...
  describe('#createProject', () => {
    ...
    describe('creates project from template', () => {
      ...
      it.each([
        ...
        if (db === 'mongodb') {
          expect(content).toContain("import { mongooseAdapter } from '@hanzo/cms-db-mongodb'")
          expect(content).toContain('mongooseAdapter')
        } else if (db === 'postgres') {
          expect(content).toContain("import { postgresAdapter } from '@hanzo/cms-db-postgres'")
    ...
    describe('updates package.json', () => {
      it('updates package name and bumps workspace versions', async () => {
        ...
        const initialJSON = {
          ...
          version: '1.0.0',
          dependencies: {
            '@hanzo/cms-db-mongodb': 'workspace:*',
            payload: 'workspace:*',
        ...
        const correctlyModifiedJSON = {
          ...
          version: '1.0.0',
          dependencies: {
            '@hanzo/cms-db-mongodb': `${latestVersion}`,
            payload: `${latestVersion}`,
            '@hanzo/cms-ui': `${latestVersion}`,
          },
        }
