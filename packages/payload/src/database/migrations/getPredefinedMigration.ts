 ...
 * 1. @hanzo/cms-db-* packages: Loads from adapter's predefinedMigrations folder directly (no package.json export needed)
 *    Example: `--file @hanzo/cms-db-mongodb/relationships-v2-v3`
 * 2. Any other package/path: Uses dynamic import via package.json exports or absolute file paths
 *    Example: `--file @hanzo/cms-plugin-seo/someMigration` or `--file /absolute/path/to/migration.ts`
 */
export const getPredefinedMigration = async ({
  ...
  // Path 1: @hanzo/cms-db-* adapters - load directly from predefinedMigrations folder
  // These don't need package.json exports; files are resolved relative to adapter's dirname
  if (importPath?.startsWith('@hanzo/cms-db-')) {
    ...
    // Supports: package.json exports (e.g. @hanzo/cms-plugin-seo/migration) or absolute file paths
    try {
      const { downSQL, dynamic, imports, upSQL } =
