...
export const createSchemaGenerator = ({
  ...
  return async function generateSchema(
    ...
    if (this.cms.config.localization && enumImport) {
      addEnum('enum__locales', this.cms.config.localization.localeCodes)
    }
  ...
  .map(
    ([key, column]) =>
      `  ${sanitizeObjectKey(key)}: ${columnToCodeConverter({
        ...
        circularEdges,
        column,
        locales: this.cms.config.localization
          ? this.cms.config.localization.localeCodes
          : undefined,
        tableKey: tableName,
      })},`,
  )
 ...
 * and re-run \`cms generate:db-schema\` to regenerate this file.
 */
`
    ...
    if (log) {
      this.cms.logger.info(`Written ${outputFile}`)
    }
  }
}
